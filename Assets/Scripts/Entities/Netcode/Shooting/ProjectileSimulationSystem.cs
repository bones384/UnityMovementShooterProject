...
namespace Entities.Netcode.Shooting
{
    ...
    public partial struct ProjectileSimulationSystem : ISystem
    {
        private static float3 ClosestPointOnSegment(float3 a, float3 b, float3 p)
        {
            ...
            var t = math.saturate(math.dot(p - a, ab) / lenSq);
            return a + t * ab;
        }
        ...
        public unsafe void OnUpdate(ref SystemState state)
        {
            ...
            var parryRange = 4.0f;
            var parryAngle = 45f;
            var parrySpeedMult = 2.0f;
            var parryDamageMult = 2.0f;
                         ...
                         .WithEntityAccess())
            {
                ...
                if (projectile.ValueRO.ApplyGravity) projectile.ValueRW.Velocity.y -= gravityMagnitude * dt;
                ...
                var wasDeflectedThisFrame = false;
                             ...
                             .WithEntityAccess())
                {
                    ...
                    var eyePos = pTransform.ValueRO.Position + new float3(0, 1.8f, 0);

                    var aimRot = math.mul(quaternion.RotateY(pLook.ValueRO.Yaw),
                        quaternion.RotateX(-pLook.ValueRO.Pitch));
                    var aimDir = math.mul(aimRot, new float3(0, 0, 1));

                    var closestPoint = ClosestPointOnSegment(currentPos, currentPos + displacement, eyePos);
                    var toProj = closestPoint - eyePos;
                    var distSq = math.lengthsq(toProj);

                    if (distSq < parryRange * parryRange)
                    {
                        var dot = math.dot(math.normalizesafe(toProj), aimDir);
                        if (dot > math.cos(math.radians(parryAngle)))
                        {
                            projectile.ValueRW.Velocity =
                                aimDir * math.length(projectile.ValueRO.Velocity) * parrySpeedMult;
                            ...
                            if (shouldLog) Debug.Log($"[{role}] VOLUME DEFLECT!");

                            var speedPitch = math.clamp(math.length(projectile.ValueRO.Velocity) * 0.05f, 1f, 3f);
                            if (networkTime.IsFirstTimeFullyPredictingTick)
                            {
                                ...
                                ecb.AddComponent(audioReq,
                                    new AudioRequest
                                    {
                                        Effect = SFX.Parry, Position = currentPos, Pitch = speedPitch,
                ...
                if (hit)
                {
                    ...
                    if (SystemAPI.HasComponent<PlayerStateComponent>(hitResult.Entity))
                    {
                        ...
                        if (targetState.IsParrying)
                        {
                            ...
                            var aimRot = math.mul(quaternion.RotateY(targetLook.Yaw),
                                quaternion.RotateX(-targetLook.Pitch));
                            var aimDir = math.mul(aimRot, new float3(0, 0, 1));
                            ...
                            if (shouldLog) Debug.Log($"[{role}] BODY DEFLECT!");
                        }
                        else
                        {
                            ...
                            if (isEnemy)
                            {
                                ...
                                {
                                    var ownerState =
                                        SystemAPI.GetComponent<PlayerStateComponent>(projectile.ValueRO.Owner);
                                ...
                                if (networkTime.IsFirstTimeFullyPredictingTick &&
                                    projectile.ValueRO.Owner != Entity.Null &&
                                    SystemAPI.HasComponent<PlayerStateComponent>(projectile.ValueRO.Owner))
                                {
                    ...
                    {
                        if (shouldLog) Debug.Log($"[{role}] Projectile HIT Wall");
                        if (networkTime.IsFirstTimeFullyPredictingTick)
                        {
                            var audioReq = ecb.CreateEntity();
                            ecb.AddComponent(audioReq,
                                new AudioRequest
                                {
                                    Effect = SFX.HitGround, Position = currentPos, Pitch = 1f, LocalTargetNetworkId = -1
                                });
                        }
                    }
