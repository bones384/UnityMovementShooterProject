...
{
    private readonly Color _colorBlue = new(0.2f, 0.5f, 0.9f);
    private readonly Color _colorBlueLight = new(0.4f, 0.7f, 1.0f);
    private readonly Color _colorGray = new(0.4f, 0.4f, 0.4f);
    private readonly Color _colorRed = new(0.8f, 0.2f, 0.2f);
    private readonly Color _colorRedLight = new(1.0f, 0.4f, 0.4f);
    private UIDocument _doc;
    private bool _hasPlayedGameOverSound;
    ...
    private void Update()
    {
        ...
        if (KothBridge.IsGameOver)
        {
            ...
            var iWon = KothBridge.WinningTeam == myTeam;
    ...
    {
        var s = math.max(0, Mathf.CeilToInt(seconds));
        var mins = s / 60;
        var secs = s % 60;
        return $"{mins}:{secs:D2}";
    }
